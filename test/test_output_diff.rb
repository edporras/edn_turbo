...
class EDN_EXT_Test < Test::Unit::TestCase
  ...
  def test_output_diff
    ...
    assert(
      EDN_EXT.read('test/map_2.edn') ==
      {:int=>1, :string=>"hello", :char=>"a", :array=>[0, 1], :hash=>{:key=>"value"}}
    )

    assert(
      EDN_EXT.read('test/map_3.edn') ==
